    ...
    return (
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>
              {t("profile.websiteUrls", "Website URLs")}
            </DialogTitle>
            <DialogDescription>
              {t(
                "profile.websiteUrlsDescription",
                "You can add up to 2 website URLs.",
              )}
            </DialogDescription>
          </DialogHeader>
...
const MyPage: React.FC = () => {
  ...
  const {
    ...
    deleteAnalysis,
    resetData,
    pendingUpdates, // 진행 중인 업데이트 상태 추가
  } = usePhotoAnalyses(user?.id, false); // 항상 편집 모드 해제
  ...
  const loaderRef = useCallback((node: HTMLDivElement | null) => {
    ...
    if (observerRef.current) {
      observerRef.current.disconnect(); // ✅ 반드시 이전 옵저버 제거
    }

    observerRef.current = new IntersectionObserver(entries => {
      const entry = entries[0];
      if (entry.isIntersecting && hasMore && !isLoading) {
        loadNextPage(); // ✅ 여기서 중복 방지를 한번 더 해도 좋음
      }
    });
  ...
  // 웹사이트 URL 업데이트
  const handleSaveSocialLinks = useCallback(
    (data: {
      websiteUrl1?: string;
      websiteUrl2?: string;
      websiteLabel1?: string;
  ...
  // 통계 데이터 계산 제거 - 통계 카드가 UI에서 제거되어 필요 없어짐
              ...
              <div className="space-y-1">
                <h1 className="text-4xl font-black leading-tight">
                  <span className="bg-gradient-to-r from-indigo-700 via-purple-700 to-indigo-700 text-transparent bg-clip-text bg-size-200 animate-gradient">
                    {userInfo.displayName || t("profile.anonymous", "User")}
                  </span>
                </h1>
                ...
                <>
                  {userInfo.websiteUrl1 && (
                    ...
                    >
                      ...
                      <span className="text-sm truncate max-w-[200px]">
                        {userInfo.websiteLabel1 ||
                          userInfo.websiteUrl1.replace(/^https?:\/\//, "")}
                      </span>
                    </a>
                  )}
                  {userInfo.websiteUrl2 && (
                    ...
                    >
                      <Icons.Globe className="h-4 w-4 mr-1" />
                      <span className="text-sm truncate max-w-[200px]">
                        {userInfo.websiteLabel2 ||
                          userInfo.websiteUrl2.replace(/^https?:\/\//, "")}
                      </span>
                    </a>
                  )}
                </>
            ...
            <div className="flex items-center justify-between gap-4 mb-6">
              ...
              <div className="flex items-center gap-2">
                <Select
                  value={sortOrder}
                  onValueChange={(value) =>
                    setSortOrder(value as "score" | "date")
                  }
                >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                {sortedAnalysisCards.map((card) => (
                  <PhotoAnalysisCard
                    ...
                    isPending={pendingUpdates[card.id]}
                  />
                ))}
