/**
 * server/photosByModelDirect.ts
 * 
 * 최적화된 카메라 모델별 사진 검색 함수 (analyses 테이블의 camera_model 칼럼 직접 활용)
 * 
 * @param cameraModel 검색할 카메라 모델명
 * @param userId 특정 사용자의 사진만 검색할 경우 사용자 ID (선택적)
 * @param includeHidden 숨겨진 사진 포함 여부 (기본값: false)
 * @param page 페이지 번호 (기본값: 1)
 * @param limit 페이지당 항목 수 (기본값: 12)
 * @returns 사진 목록과 총 개수
 */
import { db } from "./db";
import { sql } from "drizzle-orm";

export async function getPhotosByModelDirect(
  cameraModel: string, 
  userId?: number, 
  ...
  try {
    ...
    // SQL 조건 구성 (카메라 모델을 analyses 테이블과 photos 테이블 모두에서 검색)
    // analyses 테이블의 camera_model 필드 또는 photos 테이블의 exifData 필드에서 검색
    let whereCondition = sql`(
      a.camera_model = ${cameraModel} 
      OR (p.exif_data::text LIKE ${'%' + cameraModel + '%'})
    )`; 
    ...
    console.log(`[getPhotosByModelDirect] 검색 조건: 카메라 모델 "${cameraModel}" (analyses 테이블 또는 exifData 필드에서 검색)`);
    ...
    const photosWithAnalyses = await Promise.all(resultArray.map(async (row: Record<string, any>) => {
      ...
      try {
        ...
        if (photo.userId) {
          const userResults = await db.db.select({
            id: users.id,
            displayName: users.displayName
            // profilePicture 필드 제외 - 응답 크기 감소
          })
          .from(users)
        ...
        console.error(`[getPhotosByModelDirect] 사용자 정보 조회 오류 (사용자 ID: ${photo.userId}):`, error);
      }
